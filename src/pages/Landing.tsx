      ...
      <main className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 pt-6 sm:pt-8 md:pt-12 pb-16 sm:pb-20 md:pb-24">
        ...
        <section className="mt-20 sm:mt-24 md:mt-32 text-center">
          <div className="bg-gradient-to-r from-amber-200/50 to-orange-200/50 rounded-2xl sm:rounded-3xl md:rounded-[40px] p-6 sm:p-10 md:p-16">
            ...
            <p className="mt-4 text-xs text-amber-700">Free forever plan available. Upgrade anytime.</p>
          </div>
        </section>
      </main>
        ...
        {[{ emoji: '🛒', name: 'Groceries', count: 5 }, { emoji: '✅', name: 'Work Tasks', count: 3 }].map((list) => (
          <div key={list.name} className="flex items-center gap-2 bg-white rounded-xl px-3 py-2 mb-2 shadow-sm">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[8px] text-amber-700">{list.count} items</p>
            </div>
          </div>
        ))}
        ...
        <div className="flex-1 space-y-1.5">
          {[
            ...
            <div key={item.label} className="flex items-center gap-2 bg-white rounded-lg px-2.5 py-1.5 shadow-sm">
              ...
              <span className={`text-[10px] font-medium ${item.done ? 'line-through text-amber-600' : 'text-amber-900'}`}>{item.label}</span>
            </div>
          ))}
        </div>
        <div className="flex gap-1.5 mt-3">
          <div className="flex-1 bg-white rounded-lg px-2.5 py-1.5 text-[9px] text-amber-600 border border-amber-200 shadow-sm">Add an item…</div>
      ...
      <div className="p-3">
        ...
        <div className="bg-white rounded-2xl shadow-lg p-3 border border-amber-100">
          ...
          <div className="flex gap-1.5 mb-3">
            <div className="flex-1 bg-amber-50 border border-amber-200 rounded-lg px-2 py-1.5 text-[9px] text-amber-700">friend@email.com</div>
            <div className="px-2 py-1.5 bg-amber-600 text-white rounded-lg text-[9px] font-bold shadow-sm">Send</div>
          </div>
          <p className="text-[9px] text-amber-700 mb-2 font-medium">Collaborators</p>
          {['🧑 Alex', '👩 Jamie'].map((name) => (
            <div key={name} className="flex items-center gap-1.5 py-1">
              ...
              <span className="ml-auto text-[8px] text-green-700 font-medium">Active</span>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <PhoneShell>
      <div className="p-3">
        ...
        <p className="text-[8px] text-amber-700 mb-3">Alex just added an item ✨</p>
        <div className="space-y-1.5">
          {[
            ...
            <div key={item.label} className={`flex items-center gap-2 rounded-lg px-2.5 py-1.5 shadow-sm ${item.isNew ? 'bg-amber-100 border border-amber-300' : 'bg-white'}`}>
              ...
              <span className={`text-[10px] font-medium flex-1 ${item.done ? 'line-through text-amber-600' : 'text-amber-900'}`}>{item.label}</span>
